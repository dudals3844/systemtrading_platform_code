...
class Real(QAxWidget):
    ...
    def receiveNotConcludedAccount(self):
        code = self.dynamicCall("GetChejanData(int)", self.realType.REALTYPE['주문체결']['종목코드'])[1:]
        code = self.standard.standardCode(code)  # 오류안나게 종목코드 6자리 맞춰줌
    ...
    def receiveChegulPrice(self, sCode, sRealType):
    ...
    def receiveHoga(self, sCode, sRealType):
        hogaTime = self.dynamicCall("GetCommRealData(QString, int)", sCode,self.realType.REALTYPE[sRealType]['호가시간'])
        medo_1 = self.dynamicCall("GetCommRealData(QString, int)", sCode,self.realType.REALTYPE[sRealType]['매도호가1'])
        medo_1 = int(medo_1)
        medo_1_Quantity = self.dynamicCall("GetCommRealData(QString, int)", sCode,self.realType.REALTYPE[sRealType]['매도호가수량1'])

        mesu_1 = self.dynamicCall("GetCommRealData(QString, int)", sCode,self.realType.REALTYPE[sRealType]['매수호가1'])
        mesu_1 = int(mesu_1)
        mesu_1_Quantity = self.dynamicCall("GetCommRealData(QString, int)", sCode, self.realType.REALTYPE[sRealType]['매수호가수량1'])


        medo_2 = self.dynamicCall("GetCommRealData(QString, int)", sCode, self.realType.REALTYPE[sRealType]['매도호가2'])
        medo_2 = int(medo_2)
        medo_2_Quantity = self.dynamicCall("GetCommRealData(QString, int)", sCode,
                                           self.realType.REALTYPE[sRealType]['매도호가수량2'])
        mesu_2 = self.dynamicCall("GetCommRealData(QString, int)", sCode, self.realType.REALTYPE[sRealType]['매수호가2'])
        mesu_2 = int(mesu_2)
        mesu_2_Quantity = self.dynamicCall("GetCommRealData(QString, int)", sCode,
                                           self.realType.REALTYPE[sRealType]['매수호가수량2'])

        medo_3 = self.dynamicCall("GetCommRealData(QString, int)", sCode, self.realType.REALTYPE[sRealType]['매도호가3'])
        medo_3 = int(medo_3)
        medo_3_Quantity = self.dynamicCall("GetCommRealData(QString, int)", sCode,
                                           self.realType.REALTYPE[sRealType]['매도호가수량3'])
        mesu_3 = self.dynamicCall("GetCommRealData(QString, int)", sCode, self.realType.REALTYPE[sRealType]['매수호가3'])
        mesu_3 = int(mesu_3)
        mesu_3_Quantity = self.dynamicCall("GetCommRealData(QString, int)", sCode,
                                           self.realType.REALTYPE[sRealType]['매수호가수량3'])

        medo_4 = self.dynamicCall("GetCommRealData(QString, int)", sCode, self.realType.REALTYPE[sRealType]['매도호가4'])
        medo_4 = int(medo_4)
        medo_4_Quantity = self.dynamicCall("GetCommRealData(QString, int)", sCode,
                                           self.realType.REALTYPE[sRealType]['매도호가수량4'])
        mesu_4 = self.dynamicCall("GetCommRealData(QString, int)", sCode, self.realType.REALTYPE[sRealType]['매수호가4'])
        mesu_4 = int(mesu_4)
        mesu_4_Quantity = self.dynamicCall("GetCommRealData(QString, int)", sCode,
                                           self.realType.REALTYPE[sRealType]['매수호가수량4'])

        medo_5 = self.dynamicCall("GetCommRealData(QString, int)", sCode, self.realType.REALTYPE[sRealType]['매도호가5'])
        medo_5 = int(medo_5)
        medo_5_Quantity = self.dynamicCall("GetCommRealData(QString, int)", sCode,
                                           self.realType.REALTYPE[sRealType]['매도호가수량5'])
        mesu_5 = self.dynamicCall("GetCommRealData(QString, int)", sCode, self.realType.REALTYPE[sRealType]['매수호가5'])
        mesu_5 = int(mesu_5)
        mesu_5_Quantity = self.dynamicCall("GetCommRealData(QString, int)", sCode,
                                           self.realType.REALTYPE[sRealType]['매수호가수량5'])

        medo_6 = self.dynamicCall("GetCommRealData(QString, int)", sCode, self.realType.REALTYPE[sRealType]['매도호가6'])
        medo_6 = int(medo_6)
        medo_6_Quantity = self.dynamicCall("GetCommRealData(QString, int)", sCode,
                                           self.realType.REALTYPE[sRealType]['매도호가수량6'])
        mesu_6 = self.dynamicCall("GetCommRealData(QString, int)", sCode, self.realType.REALTYPE[sRealType]['매수호가6'])
        mesu_6 = int(mesu_6)
        mesu_6_Quantity = self.dynamicCall("GetCommRealData(QString, int)", sCode,
                                           self.realType.REALTYPE[sRealType]['매수호가수량6'])

        medo_7 = self.dynamicCall("GetCommRealData(QString, int)", sCode, self.realType.REALTYPE[sRealType]['매도호가7'])
        medo_7 = int(medo_7)
        medo_7_Quantity = self.dynamicCall("GetCommRealData(QString, int)", sCode,
                                           self.realType.REALTYPE[sRealType]['매도호가수량7'])
        mesu_7 = self.dynamicCall("GetCommRealData(QString, int)", sCode, self.realType.REALTYPE[sRealType]['매수호가7'])
        mesu_7 = int(mesu_7)
        mesu_7_Quantity = self.dynamicCall("GetCommRealData(QString, int)", sCode,
                                           self.realType.REALTYPE[sRealType]['매수호가수량7'])

        medo_8 = self.dynamicCall("GetCommRealData(QString, int)", sCode, self.realType.REALTYPE[sRealType]['매도호가8'])
        medo_8 = int(medo_8)
        medo_8_Quantity = self.dynamicCall("GetCommRealData(QString, int)", sCode,
                                           self.realType.REALTYPE[sRealType]['매도호가수량8'])
        mesu_8 = self.dynamicCall("GetCommRealData(QString, int)", sCode, self.realType.REALTYPE[sRealType]['매수호가8'])
        mesu_8 = int(mesu_8)
        mesu_8_Quantity = self.dynamicCall("GetCommRealData(QString, int)", sCode,
                                           self.realType.REALTYPE[sRealType]['매수호가수량8'])

        medo_9 = self.dynamicCall("GetCommRealData(QString, int)", sCode, self.realType.REALTYPE[sRealType]['매도호가9'])
        medo_9 = int(medo_9)
        medo_9_Quantity = self.dynamicCall("GetCommRealData(QString, int)", sCode,
                                           self.realType.REALTYPE[sRealType]['매도호가수량9'])
        mesu_9 = self.dynamicCall("GetCommRealData(QString, int)", sCode, self.realType.REALTYPE[sRealType]['매수호가9'])
        mesu_9 = int(mesu_9)
        mesu_9_Quantity = self.dynamicCall("GetCommRealData(QString, int)", sCode,
                                           self.realType.REALTYPE[sRealType]['매수호가수량9'])

        medo_10 = self.dynamicCall("GetCommRealData(QString, int)", sCode, self.realType.REALTYPE[sRealType]['매도호가10'])
        medo_10 = int(medo_10)
        medo_10_Quantity = self.dynamicCall("GetCommRealData(QString, int)", sCode,
                                           self.realType.REALTYPE[sRealType]['매도호가수량10'])
        mesu_10 = self.dynamicCall("GetCommRealData(QString, int)", sCode, self.realType.REALTYPE[sRealType]['매수호가10'])
        mesu_10 = int(mesu_10)
        mesu_10_Quantity = self.dynamicCall("GetCommRealData(QString, int)", sCode,
                                           self.realType.REALTYPE[sRealType]['매수호가수량10'])

        totalMedoHoga = self.dynamicCall("GetCommRealData(QString, int)", sCode, self.realType.REALTYPE[sRealType]['매도호가총잔량'])
        totalMesuHoga = self.dynamicCall("GetCommRealData(QString, int)", sCode, self.realType.REALTYPE[sRealType]['매수호가총잔량'])


        # hogaList = [[medo_10_Quantity, medo_10, ''],
        #             [medo_9_Quantity, medo_9, ''],
        #             [medo_8_Quantity, medo_8, ''],
        #             [medo_7_Quantity, medo_7, ''],
        #             [medo_6_Quantity, medo_6, ''],
        #             [medo_5_Quantity, medo_5, ''],
        #             [medo_4_Quantity, medo_4, ''],
        #             [medo_3_Quantity, medo_3, ''],
        #             [medo_2_Quantity, medo_2, ''],
        #             [medo_1_Quantity, medo_1, ''],
        #             ['',mesu_1,mesu_1_Quantity],
        #             ['',mesu_2,mesu_2_Quantity],
        #             ['',mesu_3,mesu_3_Quantity],
        #             ['',mesu_4,mesu_4_Quantity],
        #             ['',mesu_5,mesu_5_Quantity],
        #             ['',mesu_6,mesu_6_Quantity],
        #             ['',mesu_7,mesu_7_Quantity],
        #             ['',mesu_8,mesu_8_Quantity],
        #             ['',mesu_9,mesu_9_Quantity],
        #             ['',mesu_10,mesu_10_Quantity],
        #             [totalMedoHoga,'',totalMesuHoga]
        #             ]
        #
        # hogaDataFrame = pd.DataFrame(hogaList, columns=['매도수량','호가','매수수량'])
        return sCode, hogaTime, mesu_1, medo_1, mesu_1_Quantity, medo_1_Quantity, mesu_2, medo_2, mesu_2_Quantity,medo_2_Quantity,mesu_3,medo_3,mesu_3_Quantity,medo_3_Quantity,mesu_4,medo_4,mesu_4_Quantity,medo_4_Quantity,mesu_5,medo_5,mesu_5_Quantity,medo_5_Quantity,mesu_6,medo_6,mesu_6_Quantity,medo_6_Quantity,mesu_7,medo_7,mesu_7_Quantity,medo_7_Quantity,mesu_8,medo_8,mesu_8_Quantity,medo_8_Quantity,mesu_9, medo_9,mesu_9_Quantity,medo_9_Quantity, mesu_10,medo_10,mesu_10_Quantity,medo_10_Quantity,totalMesuHoga,totalMedoHoga


    def receiveJangStatus(self, sCode, sRealType):
        fid = self.realType.REALTYPE[sRealType]['장운영구분']
        value = self.dynamicCall("GetCommRealData(QString,int)", sCode, fid)
        return value

    def receiveConditionRealStock(self, strCode, strType, strConditionName, strConditionIndex):
    ...
    def requestRealCondition(self, index, conditionName):
